...
const CLIENT_BUNDLE = `document.querySelector('form')?.addEventListener('submit',()=>{const button=document.querySelector('button[type="submit"]');if(button instanceof HTMLButtonElement){button.disabled=true;button.textContent='Saving…';}});const fileInput=document.getElementById('yaml-file');const yamlInput=document.getElementById('yaml');async function loadYamlFile(){if(!(fileInput instanceof HTMLInputElement)||!(yamlInput instanceof HTMLTextAreaElement))return;const file=fileInput.files?.[0];if(!file||file.size>65536)return;yamlInput.value=await file.text();}if(fileInput instanceof HTMLInputElement)fileInput.addEventListener('change',loadYamlFile);`;
const CLIENT_BUNDLE_PATH = "/debugger/app.3be0e1e150878f6a8c789c9283dccb5b63b76b7ae610aadaa07fa15c7f14ca36.js";
const CLIENT_BUNDLE_SHA256 = "3be0e1e150878f6a8c789c9283dccb5b63b76b7ae610aadaa07fa15c7f14ca36";
