...
function debuggerActionControls(input: { selectedSession: DebugSessionRecord | null }): string {
  ...
  return `<section><h2>Manual debugger actions</h2><p>Every button sends one bounded action through the authenticated plugin UI session. Actions marked approval require this human click; the LLM cannot use this route.</p><form id="debug-actions" data-device-id="${escapeHtml(session.soulcloudDeviceRef)}" data-target-config-revision="${session.targetConfigRevision}" data-target-id="${escapeHtml(session.targetId)}">${buttons}<label for="debug-start-mode">Start mode</label><select id="debug-start-mode"><option value="automatic">Automatic</option><option value="assisted">Assisted</option></select><button type="submit" data-debug-action="debug.start">Start target (approval)</button><p id="debug-action-status" role="status" aria-live="polite"></p></form></section>`;
}
