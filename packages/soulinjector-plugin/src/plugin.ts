...
const yamlFileStatus=document.getElementById('yaml-file-status');
function setYamlFileStatus(message){if(yamlFileStatus instanceof HTMLElement)yamlFileStatus.textContent=message;}
async function loadYamlFile(){if(!(fileInput instanceof HTMLInputElement)||!(yamlInput instanceof HTMLTextAreaElement))return;const file=fileInput.files?.[0];if(!file)return;if(file.size>65536){setYamlFileStatus('YAML file must be no larger than 64 KiB.');fileInput.value='';return;}try{const text=await file.text();if(text.length>65536){setYamlFileStatus('YAML file must be no larger than 64 KiB.');fileInput.value='';return;}yamlInput.value=text;setYamlFileStatus('YAML loaded. Review it before saving.');}catch{setYamlFileStatus('Unable to read the YAML file.');}}
...
let debugExecutionState='unknown';
async function cancelDebugCommand(executionId,commandId,button){if(!(button instanceof HTMLButtonElement))return;button.disabled=true;button.textContent='Cancelling…';try{const response=await fetch('/plugins/'+encodeURIComponent(artifactInstallation)+'/debugger/executions/'+encodeURIComponent(executionId)+'/commands/'+encodeURIComponent(commandId)+'/cancel',{method:'POST'});if(!response.ok){button.disabled=false;button.textContent='Cancel';return;}await refreshCommandTimeline();}catch{button.disabled=false;button.textContent='Cancel';}}
function setDebugExecutionControls(enabled,message){if(actionForm instanceof HTMLFormElement)for(const control of actionForm.querySelectorAll('button'))control.disabled=!enabled;if(releaseButton instanceof HTMLButtonElement)releaseButton.disabled=!enabled;if(!enabled&&typeof message==='string')setActionStatus(message);}
async function refreshDebugExecutionState(){if(!(releaseButton instanceof HTMLButtonElement))return;const executionId=releaseButton.dataset.executionId;if(typeof executionId!=='string'||executionId.length===0)return;try{const response=await fetch('/plugins/'+encodeURIComponent(artifactInstallation)+'/debugger/executions/'+encodeURIComponent(executionId));if(!response.ok){debugExecutionState='unavailable';setDebugExecutionControls(false,'Debugger execution is no longer available.');return;}const record=await response.json();if(!record||typeof record!=='object')return;const state=String(record.state??'');debugExecutionState=state;const active=state==='active'&&typeof record.deviceLeaseExpiresAt==='string';if(!active)setDebugExecutionControls(false,state==='paused'?'Debugger execution is paused; device actions are disabled.':'Debugger execution is '+(state||'unavailable')+'; device actions are disabled.');}catch{} }
async function refreshCommandTimeline(){if(!(commandTimeline instanceof HTMLElement))return;const executionId=commandTimeline.dataset.executionId;if(typeof executionId!=='string'||executionId.length===0)return;try{const response=await fetch('/plugins/'+encodeURIComponent(artifactInstallation)+'/debugger/executions/'+encodeURIComponent(executionId)+'/commands');if(!response.ok)return;const commands=await response.json();if(!Array.isArray(commands))return;const list=document.createElement('ol');for(const command of commands){if(!command||typeof command!=='object')continue;const item=document.createElement('li');const record=command;item.textContent='batch '+String(record.batchId??'')+' — '+String(record.state??'')+(record.resultCode===null||record.resultCode===undefined?'':' — result '+String(record.resultCode));if((debugExecutionState==='active'||debugExecutionState==='paused'||debugExecutionState==='cancelling')&&(record.state==='queued'||record.state==='leased'||record.state==='broker_accepted')){const button=document.createElement('button');button.type='button';button.textContent='Cancel';button.addEventListener('click',function(){void cancelDebugCommand(executionId,String(record.id??''),button);});item.append(' ',button);}list.appendChild(item);}commandTimeline.replaceChildren(list);}catch{} }
