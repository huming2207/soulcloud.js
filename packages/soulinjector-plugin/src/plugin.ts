...
const CLIENT_BUNDLE = `document.querySelector('form')?.addEventListener('submit',()=>{const button=document.querySelector('button[type="submit"]');if(button instanceof HTMLButtonElement){button.disabled=true;button.textContent='Saving…';}});const fileInput=document.getElementById('yaml-file');const yamlInput=document.getElementById('yaml');async function loadYamlFile(){if(!(fileInput instanceof HTMLInputElement)||!(yamlInput instanceof HTMLTextAreaElement))return;const file=fileInput.files?.[0];if(!file||file.size>65536)return;yamlInput.value=await file.text();}if(fileInput instanceof HTMLInputElement)fileInput.addEventListener('change',loadYamlFile);const artifactForm=document.getElementById('artifact-upload');const artifactFile=document.getElementById('artifact-file');const artifactKind=document.getElementById('artifact-kind');const artifactCase=document.getElementById('artifact-case');const artifactStatus=document.getElementById('artifact-upload-status');const artifactInstallation=location.pathname.split('/')[2]??'';function setArtifactStatus(message){if(artifactStatus instanceof HTMLElement)artifactStatus.textContent=message;}async function sendArtifactUpload(url,file,uploadId){return await fetch(url,{method:'POST',headers:{'content-type':'application/octet-stream','idempotency-key':uploadId,'x-soulcloud-content-length':String(file.size)},body:file});}async function uploadArtifact(event){event.preventDefault();if(!(artifactFile instanceof HTMLInputElement)||!(artifactKind instanceof HTMLSelectElement))return;const file=artifactFile.files?.[0];const kind=artifactKind.value;if(!file||(kind!=='elf'&&kind!=='firmware')){setArtifactStatus('Choose a file and artifact type.');return;}if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(file.name)){setArtifactStatus('Filename must use letters, numbers, dot, dash or underscore.');return;}const contentType=file.type||(kind==='elf'?'application/x-elf':'application/octet-stream');const selectedCase=artifactCase instanceof HTMLSelectElement?artifactCase.value:'';const caseQuery=selectedCase?'&case_id='+encodeURIComponent(selectedCase):'';const url='/plugins/'+encodeURIComponent(artifactInstallation)+'/debugger/artifacts?kind='+encodeURIComponent(kind)+'&filename='+encodeURIComponent(file.name)+'&content_type='+encodeURIComponent(contentType)+caseQuery;const uploadId=crypto.randomUUID();setArtifactStatus('Uploading…');let response;try{response=await sendArtifactUpload(url,file,uploadId);}catch{setArtifactStatus('Retrying…');try{response=await sendArtifactUpload(url,file,uploadId);}catch{setArtifactStatus('Upload failed');return;}}if(!response.ok){setArtifactStatus('Upload failed ('+response.status+')');return;}setArtifactStatus('Upload complete');location.reload();}if(artifactForm instanceof HTMLFormElement)artifactForm.addEventListener('submit',uploadArtifact);`;
const CLIENT_BUNDLE_PATH = "/debugger/app.ecb6bd1fbb95f75e936406aea7eb52b1a80c078c5fbb8436ca87b1425f75779d.js";
const CLIENT_BUNDLE_SHA256 = "ecb6bd1fbb95f75e936406aea7eb52b1a80c078c5fbb8436ca87b1425f75779d";
