...
/** Renew a human-controlled lease with scope authorization in the mutation transaction. */
export async function renewDebugExecutionLeaseForUser(
  prisma: PrismaClient,
  input: DebugExecutionUserScope & { leaseMs: number },
): Promise<DebugExecutionRecord> {
  ...
  if (!Number.isSafeInteger(input.leaseMs) || input.leaseMs < DEBUG_EXECUTION_MIN_LEASE_MS || input.leaseMs > DEBUG_EXECUTION_MAX_LEASE_MS) {
    throw new RangeError("leaseMs is outside the supported range");
  }
  return prisma.$transaction(async (tx) => {
    ...
    const rows = await tx.$queryRaw<RawExecutionRow[]>`
      UPDATE debug_executions
      SET device_lease_expires_at = LEAST(expires_at, CURRENT_TIMESTAMP + (${input.leaseMs} * INTERVAL '1 millisecond')),
          updated_at = CURRENT_TIMESTAMP
