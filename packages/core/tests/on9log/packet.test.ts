...
describe("M13: BOOT packets and strictness", () => {
  test("parses BOOT packets with opaque payload", () => {
    // type 3 in the high nibble (0x30)
    const bytes = new Uint8Array([
      0x9a, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0xff, 0xff, // streaming
      0xde, 0xad, 0xbe, 0xef, // opaque boot payload
    ]);
    const packet = parseOn9logPacket(bytes);
    expect(packet.kind).toBe("boot");
    if (packet.kind === "boot") {
      expect(packet.header.type).toBe(On9logPacketType.Boot);
      expect(packet.payload).toEqual(new Uint8Array([0xde, 0xad, 0xbe, 0xef]));
    }
  });

  test("rejects DROPPED packets with trailing bytes", () => {
    // payload_len 8 but DROPPED payload is only 4 bytes
    const bytes = new Uint8Array([
      0x9a, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x08, 0x00,
      0x2a, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00,
    ]);
    expect(() => parseOn9logPacket(bytes)).toThrow(/trailing bytes/);
  });

  test("rejects TIME_SYNC packets with trailing bytes", () => {
    const bytes = new Uint8Array([
      0x9a, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  test("rejects BUFFER chunks exceeding the declared total", () => {
    // total 5, offset 0, chunk_len 9 (> total)
    const bytes = new Uint8Array([
      0x9a, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    expect(() => parseOn9logPacket(bytes)).toThrow(/exceeds total length/);
  });

  test("rejects LOG packets with invalid level nibble", () => {
    // type 0, level 9 (0x09)
    const bytes = new Uint8Array([
      0x9a, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00,
    ]);
    expect(() => parseOn9logPacket(bytes)).toThrow(/invalid on9log level/);
  });
});
