...
export const artifactChunkInput = operation.extend({ installationId: z.string().uuid(), projectId: z.string().uuid(), userId: z.string().uuid(), uploadId: z.string().uuid(), caseId: z.string().uuid().optional(), kind: z.enum(["elf", "firmware"]), filename: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/), contentType: z.string().min(1).max(128), totalSize: z.number().int().positive().max(64 * 1024 * 1024), offset: z.number().int().nonnegative().max(64 * 1024 * 1024), final: z.boolean(), chunk: z.instanceof(Blob).refine((value) => value.size > 0 && value.size <= 64 * 1024, "artifact chunk must be 1..65536 bytes") }).strict()
...
export const artifactReadChunkInput = operation.extend({
...
export const artifactReadChunkOutput = z.object({
  artifactId: z.string().uuid(),
  offset: z.number().int().nonnegative().max(64 * 1024 * 1024),
  totalSize: z.number().int().positive().max(64 * 1024 * 1024),
  sha256: z.string().regex(/^[0-9a-f]{64}$/),
  chunk: z.instanceof(Blob).refine((value) => value.size > 0 && value.size <= 64 * 1024, "artifact chunk must be 1..65536 bytes"),
  final: z.boolean(),
}).strict();
...
export const managerToPluginContract = {
  ...
  debugger: {
    ...
    readArtifactChunk: procedure(artifactReadChunkInput, artifactReadChunkOutput, ["debugger", "readArtifactChunk"]),
...
export type ArtifactReadChunkInput = z.infer<typeof artifactReadChunkInput>;
export type ArtifactReadChunkOutput = z.infer<typeof artifactReadChunkOutput>;
