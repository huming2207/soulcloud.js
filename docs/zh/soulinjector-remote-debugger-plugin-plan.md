...
- SSR debugger 页面现在提供 ELF/firmware 文件选择和上传入口。浏览器只把文件流发送到
  ...
  UI 上传开始时以及提交最后一个 chunk 前还会重新校验 session 对应的 installation 快照和当前 project membership；用户在流式上传期间被撤权时，旧页面不能提交完整 artifact。
...
- ELF/firmware 的大小、文件名、ELF magic、SHA-256 校验，以及 ELF32/ELF64、字节序、machine、
  ...
  同时保留完整 artifact 与所有 chunk 的第二份大内存副本；即使 ELF header 跨越多个小 chunk，
...
- Plugin Manager 自身的 send queue、pending request 或本地容量达到上限时，事件只会延后重试，
  不消耗 delivery attempt，也不把健康 plugin 计入 installation circuit breaker；只有 plugin
  transport timeout、disconnect 或 crash 才按瞬时 plugin 故障处理。
...
cancel/take-over 与 plugin 重启后的 capability 恢复、LLM harness，以及独立 plugin-origin
