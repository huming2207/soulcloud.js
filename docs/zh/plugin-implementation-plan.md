...
runtime 已有 `ui.asset`，并通过 Manager 同源代理一个最小、content-hashed client bundle。独立
plugin-origin 的 Web POST/跳转已接入，bootstrap nonce 已使用 PostgreSQL
原子单次消费和过期清理；`context.ui.getData` 和实时 UI channel 仍未完成。
