...
| [plugin-rpc-protocol.md](plugin-rpc-protocol.md) | Dispatcher ↔ Plugin Host 的 oRPC v2/WebSocket 双向 RPC 协议、资源治理和部署约束（已实施） |
...
- **进程间通信**: 核心进程之间仅 PostgreSQL（持久化出站队列（outbox）+ 租约轮询；
  ...
  oRPC v2 双向 WebSocket（`PLUGIN_HOST_ENDPOINTS`）；插件 RPC 不依赖 MessagePack。
