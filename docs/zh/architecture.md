...
- **独立扩缩容**: API 与消息代理是独立进程，可以各自独立扩容（例如反代
  后的多个 API 副本）。
- **单消息代理约束**: 消息代理当前以**单进程**运行。持久化命令/OTA 队列
  使用 PostgreSQL 租约锁定（lease locking）（`FOR UPDATE SKIP LOCKED`），
  数据层跨实例安全；但 Aedes 会话状态（连接、订阅）是进程本地的——第二个
  代理实例可能为连接在另一实例上的设备租约命令、误判离线并退避（正确但慢
  且增加数据库抖动），实例重启还会丢失全部活跃会话。在宣传消息代理水平
  扩展之前，需要设备到代理的归属/分区方案或集群感知的代理。
