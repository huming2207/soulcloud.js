...
平台 execution 已失效，可调用仅 Manager 使用的 `debugger.abortSession`。正常情况下请求携带
`sessionId`；如果 plugin 已经提交私有 session 但 bootstrap 响应丢失，`sessionId` 可以省略或为
`null`，plugin 必须使用 installation/project/device 与唯一 `executionId` scope 找回该 session
并将其标为 failed。该清理不发送设备 command，也不绕过 Human API 的人工批准。Human API、Browser、通用
...
PLUGIN_UI_SESSION_TTL_SECONDS=...
PLUGIN_MANAGER_RECONNECT_MS=...
PLUGIN_RPC_AUTH_TOKEN=...

PLUGIN_RPC_MAX_FRAME_BYTES=...
PLUGIN_RPC_MAX_OPERATIONS=...
PLUGIN_RPC_MAX_PENDING_REQUESTS=...
PLUGIN_RPC_MAX_VALUE_DEPTH=...
PLUGIN_RPC_MAX_VALUE_NODES=...
PLUGIN_RPC_MAX_ARRAY_ITEMS=...
PLUGIN_RPC_MAX_STRING_BYTES=...
PLUGIN_RPC_MAX_BLOBS=...
PLUGIN_RPC_MAX_BLOB_BYTES=...
PLUGIN_RPC_MAX_TOTAL_BLOB_BYTES=...
...
PLUGIN_RPC_MAX_PLUGIN_CALL_DEPTH=...
PLUGIN_RPC_MAX_STAGED_COMMANDS=...
PLUGIN_RPC_MAX_STAGED_COMMAND_BYTES=...
...
PLUGIN_MAINTENANCE_INTERVAL_MS=...
PLUGIN_RETENTION_BATCH_SIZE=...
PLUGIN_RETENTION_MAX_BATCHES=...

# HTML size is not configurable here; the oRPC contract caps one rendered
# fragment at a fixed byte limit, and the deployment cannot raise it.
PLUGIN_SSR_TIMEOUT_MS=...
PLUGIN_SSR_MAX_CONCURRENCY=...

PLUGIN_ARTIFACT_MAX_BYTES=...
PLUGIN_ARTIFACT_UPLOAD_TIMEOUT_MS=...
```

具体默认值由现有实测和部署规模确定，不属于 wire protocol；`.env.example`
与 `compose.yaml` 中的当前默认值即为可运行的起点。
